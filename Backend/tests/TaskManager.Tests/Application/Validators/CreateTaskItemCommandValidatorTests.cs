...
namespace TaskManager.Tests.Application.Validators
{
    public class CreateTaskItemCommandValidatorTests
    {
        ...
        {
            var command = new CreateTaskItemCommand { Title = "Título válido", Description = "Desc", DueDate = DateTime.MinValue, UserId = Guid.NewGuid() }; // DateTime.MinValue para simular vazio/inválido
            var result = _validator.TestValidate(command);
        ...
        {
            var command = new CreateTaskItemCommand { Title = "Título válido", Description = "Desc", DueDate = DateTime.Today.AddDays(-1), UserId = Guid.NewGuid() };
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(x => x.DueDate)
        ...
        {
            var command = new CreateTaskItemCommand { Title = "Tarefa válida", Description = "Descrição válida", DueDate = DateTime.Today.AddDays(1), UserId = Guid.NewGuid() };
            var result = _validator.TestValidate(command);
